...
if __name__ == '__main__':
    ...
    ##################################################################################################
        ...
        fname = "/data/users/c3471tl/rss_regressor/traindata_" + basename
        datasetall = joblib.load(fname)
        print("loaded", fname, "!")

        anum = 3
        tmptraindata = []
        for dataset in datasetall:
            tmptraindata.append(dataset[anum])

        traindata = np.concatenate(tmptraindata, axis=0)
        ...
        for node in nodelist:
            model = Sequential()
            model.add(Dense(800, input_dim=xtrain.shape[1], activation='relu'))
            ...
            print("node = {}, train rmse = {:.4f}, test rmse = {:.4f}".format(node, train_rmse, test_rmse))
            # trainrmselist.append(train_rmse)
            # testrmselist.append(test_rmse)

            print(model.summary())
            model.save("/data/users/c3471tl/rss_regressor/train_model_n800-" + str(node) + ".h5")
            joblib.dump(xscaler, "/data/users/c3471tl/rss_regressor/train_scaler_n800-" + str(node) + ".save")
            print("saved!")
    ...
    ##################################################################################################
        ...
        tmptestdata = []
        for dataset in datasetall:
            tmptestdata.append(dataset[anum])
        testdata = np.concatenate(tmptestdata, axis=0)

        xdata = testdata[:, :-2]
        ydata = testdata[:, -2:]

        for node in nodelist:
            model = load_model("/data/users/c3471tl/rss_regressor/train_model_n800-" + str(node) + ".h5")
            xscaler = joblib.load("/data/users/c3471tl/rss_regressor/train_scaler_n800-" + str(node) + ".save")
