...
if __name__ == '__main__':
    ...
    ##################################################################################################
        ...
        fname = "/data/users/c3471tl/rss_regressor/traindata_" + basename
        traindataset = joblib.load(fname)
        print("loaded", fname, "!")
        ...
        for anum in anum_list:
            ...
            for count in range(10):
                ...
                print(model.summary())
                model.save("/data/users/c3471tl/rss_regressor/train_model11_anum" + str(anum) + "_c" + str(count) + ".h5")
                joblib.dump(xscaler, "/data/users/c3471tl/rss_regressor/train_scaler11_anum" + str(anum) + "_c" +
                            str(count) + ".save")
                print("saved!")
                ...
                xtestdata = testdata[:, :data_idx]
                ytestdata = testdata[:, -2:]

                # for node in nodelist:
                model = load_model("/data/users/c3471tl/rss_regressor/train_model11_anum" + str(anum) + "_c" + str(count)
                                   + ".h5")
                xscaler = joblib.load("/data/users/c3471tl/rss_regressor/train_scaler11_anum" + str(anum) + "_c" +
                                      str(count) + ".save")
