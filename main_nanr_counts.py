...
if __name__ == '__main__':
    ...
    ##################################################################################################
        ...
        for anum in anum_list:
            trainfname = "/data/users/c3471tl/rss_regressor/traindata_" + basename
            traindataset = joblib.load(trainfname)
            print("loaded", trainfname, "!")
            traindata = traindataset[anum]

            xtraindata = traindata[:, :-2]
            ytraindata = traindata[:, -2:]
            xscaler = MinMaxScaler(feature_range=(-1, 1))
            xscaled = xscaler.fit_transform(xtraindata)

            # separate data into train and test group
            testratio = 0.3
            valratio = 0.3
            xtmp, xtest, ytmp, ytest = train_test_split(xscaled, ytraindata, test_size=testratio, shuffle=True, random_state=7)
            ...
            for count in range(10):
                ...
                print(model.summary())
                model.save("/data/users/c3471tl/rss_regressor/train_model_anum" + str(anum) + "_c" + str(count) + ".h5")
                joblib.dump(xscaler, "/data/users/c3471tl/rss_regressor/train_scaler_anum" + str(anum) + "_c" +
                            str(count) + ".save")
                print("saved!")
                ...
                testfname = "/data/users/c3471tl/rss_regressor/testdata_" + basename
                ...
                model = load_model("/data/users/c3471tl/rss_regressor/train_model_anum" + str(anum) + "_c" + str(count)
                                   + ".h5")
                xscaler = joblib.load("/data/users/c3471tl/rss_regressor/train_scaler_anum" + str(anum) + "_c" +
                                      str(count) + ".save")
                xscaled = xscaler.transform(xtestdata)

                apostestx = xscaled[:, :anum]
                apostesty = xscaled[:, anum:(2 * anum)]
                tpostestx = ytestdata[:, 0]
                tpostesty = ytestdata[:, 1]

                # deep learning
                mse_dl = model.evaluate(xscaled, ytestdata, verbose=0)
