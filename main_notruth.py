...
if __name__ == '__main__':
    ...
    ##################################################################################################
        ...
        for num_sample in sample_list:
            xdata = traindata[:num_sample, :-2]
            ydata = traindata[:num_sample, -2:]
            ...
            print("no. of samples = {}, train rmse = {:.4f}, test rmse = {:.4f}".format(num_sample, train_rmse,
                                                                                        test_rmse))
            # trainrmselist.append(train_rmse)
            # testrmselist.append(test_rmse)

            print(model.summary())
            model.save("/data/users/c3471tl/rss_regressor/train_model_nsample" + str(num_sample) + ".h5")
            joblib.dump(xscaler, "/data/users/c3471tl/rss_regressor/train_scaler_nsample" + str(num_sample) + ".save")
            print("saved!")
            ...
            # for node in nodelist:
            model = load_model("/data/users/c3471tl/rss_regressor/train_model_nsample" + str(num_sample) + ".h5")
            xscaler = joblib.load("/data/users/c3471tl/rss_regressor/train_scaler_nsample" + str(num_sample) + ".save")
